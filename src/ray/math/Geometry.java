...
public class Geometry {
	...
	{
		// samples first on a disk (projection of the hemisphere onto the plane below) 
		//and then projects the points up onto the hemisphere
		double phi = 2 * Math.PI * seed.y;
		outDir.x = Math.sqrt(seed.x) * Math.cos(phi);
		outDir.y = Math.sqrt(seed.x) * Math.sin(phi);
